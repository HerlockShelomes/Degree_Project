...
with open(log_path, 'r') as f:
    for line in f:
        ...
        if 'coco/bbox_mAP_s' in data:
            maps.append(data['coco/bbox_mAP_s'])
            epochs.append(data['step']) # 或者用 data['epoch']

plt.plot(epochs, maps)
plt.title('mAP_s Curve')
plt.xlabel('Epoch')
plt.ylabel('mAP_s')
plt.savefig('/data/fxy_projects/mmdetection-main/graduate_result/fcos_distill_mAP_s.png')
